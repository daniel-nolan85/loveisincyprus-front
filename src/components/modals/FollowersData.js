...
const FollowersData = ({
  users,
  followersDataModalIsOpen,
  setFollowersDataModalIsOpen,
  username,
}) => {
  ...
  return (
    <Modal
      isOpen={followersDataModalIsOpen}
      onRequestClose={() => setFollowersDataModalIsOpen(false)}
      style={modalStyles}
      ...
      {users && users.length && (
        <div ref={handleModalContentRef}>
          <h2 className='center'>
            ...
            {users.length == 1 ? 'follower' : 'followers'}
          </h2>
          {users.length > 0 &&
            users.map((u) => (
