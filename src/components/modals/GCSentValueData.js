...
const GCSentValueData = ({
  cards,
  gCSentValueDataModalIsOpen,
  setGCSentValueDataModalIsOpen,
  username,
}) => {
  ...
  useEffect(() => {
    if (Object.keys(cards).length !== 0) {
      const totalCardsSentValue = cards.reduce(
        (acc, curr) => {
      ...
      setTotalCardsValue(totalCardsSentValue.totalAmount);
    }
  }, [cards]);
  ...
  return (
    <Modal
      isOpen={gCSentValueDataModalIsOpen}
      onRequestClose={() => setGCSentValueDataModalIsOpen(false)}
      style={modalStyles}
...
export default GCSentValueData;
