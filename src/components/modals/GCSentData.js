...
const GCSentData = ({
  cards,
  gCSentDataModalIsOpen,
  setGCSentDataModalIsOpen,
  username,
}) => {
  ...
  useEffect(() => {
    if (Object.keys(cards).length !== 0) {
      const totalCardsSent = cards.reduce(
        (acc, curr) => {
      ...
      setTotalCards(totalCardsSent.count);
    }
  }, [cards]);
  ...
  return (
    <Modal
      isOpen={gCSentDataModalIsOpen}
      onRequestClose={() => setGCSentDataModalIsOpen(false)}
      style={modalStyles}
      ...
      {cards && cards.length && (
        <div ref={handleModalContentRef}>
          <h2 className='center'>
            ...
            {totalCards == 1 ? 'gift card' : 'gift cards'} to{' '}
            <span
              style={{ color: '#ef5b85', fontWeight: 'bold', fontSize: '24px' }}
            >
              {cards.length}
