...
const GCReceivedData = ({
  cards,
  gCReceivedDataModalIsOpen,
  setGCReceivedDataModalIsOpen,
  username,
}) => {
  ...
  useEffect(() => {
    if (Object.keys(cards).length !== 0) {
      const totalCardsReceived = cards.reduce(
        (acc, curr) => {
      ...
      setTotalCards(totalCardsReceived.count);
    }
  }, [cards]);
  ...
  return (
    <Modal
      isOpen={gCReceivedDataModalIsOpen}
      onRequestClose={() => setGCReceivedDataModalIsOpen(false)}
      style={modalStyles}
    ...
    >
      {cards && cards.length && (
        <div ref={handleModalContentRef}>
          <h2 className='center'>
            {username} has currently received
            <br />
            <span
            ...
            {totalCards == 1 ? 'gift card' : 'gift cards'} from{' '}
            <span
              style={{ color: '#ef5b85', fontWeight: 'bold', fontSize: '24px' }}
            >
              {cards.length}
