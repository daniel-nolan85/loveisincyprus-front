          ...
          {analysing ? (
            <>
              <h2>
                ...
                {userToAnalyse.name || userToAnalyse.email.split('@')[0]} now
              </h2>
            ...
            <>
              <div className='match-images'>
                <img
                  ...
                  alt={`${
                    userToAnalyse.name || userToAnalyse.email.split('@')[0]
                  }'s profile picture`}
                />
                <h1
                  className={`score ${
              ...
              <h2>
                You and{' '}
                {userToAnalyse.name || userToAnalyse.email.split('@')[0]} have a
              ...
              >
                <ul>
                  {genderYouWant && (
                    <li>
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      fits the gender you are looking for
                    </li>
                  )}
                  {genderTheyWant && (
                    <li>
                  ...
                  {ageYouWant && (
                    <li>
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      is {userToAnalyse.age} years old, and fits the age you are
                      looking for
                    </li>
                  )}
                  {ageTheyWant && (
                  ...
                  {location && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      both live in {user.location}
                    </li>
                  )}
                  {theyWillRelocate && (
                    <li>
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      might be willing to relocate to your area
                    </li>
                  )}
                  {youWillRelocate && (
                    <li>
                  ...
                  {relocate && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      both might be willing to relocate
                    </li>
                  )}
                  {relWanted && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      are both looking for {user.relWanted}
                    </li>
                  )}
                  {language && (
                    <li>
                  ...
                  {drinks && user.drinks !== 'never' && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      both like to drink {user.drinks}
                    </li>
                  )}
                  {drinks && user.drinks === 'never' && (
                    <li>
                  ...
                  {occupation && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      both work as a {user.occupation}
                    </li>
                  )}
                  {politics && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      have the same views on politics
                    </li>
                  )}
                  {religion && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      have the same feelings about religion
                    </li>
                  )}
                  {foods && (
                    <li>
                  ...
                  {livesWith && user.livesWith !== 'alone' && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      both live with your {user.livesWith}
                    </li>
                  )}
                  {livesWith && user.livesWith === 'alone' && (
                    <li>
                  ...
                  {roleInLife && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      both believe your role in life is to focus on your{' '}
                      {user.roleInLife}
                    </li>
                  )}
                  {managesEdu && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      both believe your children's {user.managesEdu} should
                  ...
                  {sexFrequency && user.sexFrequency !== 'never' && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      both like to have sex {user.sexFrequency}
                    </li>
                  )}
                  {sexFrequency && user.sexFrequency === 'never' && (
                    <li>
                  ...
                  {treatSelf && treatSelf.length > 0 && (
                    <li>
                      You and{' '}
                      {userToAnalyse.name || userToAnalyse.email.split('@')[0]}{' '}
                      both enjoy treating yourselves with{' '}
                      {treatSelf.length > 1
